	...
	<div class="specialists-data-table mt-10">
		...
		>
				<v-form @submit.prevent="updateSpeciality">
					<v-card>
						<v-card-title><span class="headline">Редактировать специальность</span></v-card-title>
						<v-card-text>
							<v-row no-gutters>
								<v-text-field
		...
		>
			<v-card>
				...
				<v-card-text>Вы действительно хотите удалить специализацию <strong>{{deletingItem.title}}</strong>?</v-card-text>
				<v-card-actions>
					<v-spacer/>
					<v-btn
							text
...
export default class SpecialistsDataTable extends Vue {
	...
	get isEditButtonDisabled() {
		return this.editFocus ? !this.editSpeciality : this.editError ? true : !this.editSpeciality && this.origSpec !== this.editSpeciality && !this.editFetching;
	}
	...
	async updateSpeciality() {
		...
		await this.$api.specialities.put('', {
			title: this.editSpeciality,
			uid: this.editUid
		}).then(async () => {
	...
	async deleteSpeciality() {
		...
		await this.$api.specialities.delete(`/${this.deletingItem.uid}`)
							.then(({ data }) => eventBus.$emit(SHOW_ALERT, data.message, {
								type: 'success'
							}))
							.catch(err => eventBus.$emit(SHOW_ALERT, err.response.data?.message, {
								type: 'error'
