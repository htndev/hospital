<template>
  <div>
    ...
    <v-dialog persistent max-width="400px" v-model="showDeleteDialog">
      <v-card>
        ...
        <v-card-text>
          ...
          <strong>{{callbackPersonFullname}}</strong>?
        </v-card-text>
        <v-card-actions>
          <v-spacer />
    ...
    <v-dialog v-model="showBookDialog" persistent max-width="400px">
      <v-card>
          ...
          <v-row no-gutters>
            <v-col cols="5">
              ...
              >
                ...
                <v-date-picker v-model="callbackPerson.date" no-title scrollable :min="minDate">
                  <v-spacer></v-spacer>
            ...
            <v-col cols="5" offset="1">
              ...
              >
                ...
                <v-time-picker v-model="callbackPerson.time" format="24hr">
                  <v-spacer></v-spacer>
        ...
        <v-card-text>
          <v-textarea v-model.trim="callbackPerson.comment" label="Комментарий" no-resize solo />
        </v-card-text>
        <v-card-actions>
          <v-spacer />
          <v-btn @click="showBookDialog = false" text>Закрыть</v-btn>
          <v-btn @click="book" text :loading="bookButtonLoading">Записать</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <v-data-table
    ...
    >
      <template #item.action="{item}">
        <v-icon left small @click="openBookDialog(item)">mdi-pencil</v-icon>
        <v-icon right small @click="openDeleteDialog(item)">mdi-delete</v-icon>
      </template>
    </v-data-table>
  </div>
</template>
...
export default class CallbackDataTable extends Vue {
  ...
  get callbackSpeciality() {
    return this.callbackPerson.speciality
      ? this.callbackPerson.speciality.title
      : '';
  }
  ...
  async book() {
    const acceptanceDate = new Date(
      `${this.callbackPerson.date} ${this.callbackPerson.time}`
    );
    const body = {
  ...
  async deleteBook() {
    ...
    await this.$api.callback
      .delete(`/${this.callbackPerson._id}`)
      .then(({ data }) =>
        eventBus.$emit(SHOW_ALERT, data.message, { type: 'success' })
      )
      .catch(({ response }) =>
