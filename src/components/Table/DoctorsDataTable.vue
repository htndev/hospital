<template>
  <div class="table-wrapper">
    ...
    <v-dialog v-model="showEditingDialog" persistent max-width="400px">
      <v-form @submit.prevent="updateDoctor">
        <v-card>
          <v-card-title>
            <span class="headline">Редактировать врача</span>
          </v-card-title>
          <v-card-text>
            <v-row no-gutters>
              <v-text-field v-model.trim="editingDoctor.name" label="Имя" outlined />
            </v-row>
            <v-row no-gutters>
              <v-text-field v-model.trim="editingDoctor.surname" label="Фамилия" outlined />
            </v-row>
            <v-row no-gutters>
              <v-text-field v-model.trim="editingDoctor.patronymics" label="Отчество" outlined />
            </v-row>
            <v-row no-gutters>
              <v-file-input
    ...
    <v-dialog v-model="showDeletingDialog" persistent max-width="400px">
      <v-form>
        <v-card>
          <v-card-title class="headline">Удаление</v-card-title>
          <v-card-text>
            Вы действительно хотите удалить
            <strong>{{getDeletingDoctorFullname}}</strong>?
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn text @click="showDeletingDialog = false">Закрыть</v-btn>
            <v-btn text color="red" :loading="deleteButtonLoading" @click="deleteDoctor">Удалить</v-btn>
          </v-card-actions>
        </v-card>
      </v-form>
    </v-dialog>
      ...
      >
        ...
        <template #item.specialities="{item}">
          <div class="speciality-wrapper">
            <v-chip-group v-if="item.specialities.length" column :show-arrows="false">
              <v-chip
        ...
        <template #item.actions="{item}">
          ...
          <v-icon small @click="setDeletingDoctor(item)">mdi-delete</v-icon>
        </template>
      </v-data-table>
    </div>
  </div>
...
export default class DoctorsDataTable extends Vue {
  ...
  async editDoctor(item: any) {
    ...
    this.specialities = specs.map((spec: any) => ({
      text: spec.title,
      value: spec.uid
    }));
    ...
    this.editingDoctor.specialities = item.specialities.map((spec: any) => ({
      value: spec.uid,
      text: spec.title
    }));
  ...
  async deleteDoctor() {
    ...
    await this.$api.doctor
      ...
      .catch(({ response }: any) =>
        eventBus.$emit(SHOW_ALERT, response.data?.message, {
          type: 'error'
        })
      );
  ...
  async updateDoctor() {
    ...
    await this.$api.doctor
      .put('', fd)
      .then(() => {
        eventBus.$emit(SHOW_ALERT, 'Врач успешно добавлен.', {
          type: 'success'
        });
      })
      .catch(err =>
