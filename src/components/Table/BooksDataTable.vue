	...
	<div>
		...
		>
			<v-card>
				...
				<v-card-text>Вы уверены, что хотите отменить прием <strong>{{editingPersonFullname}}</strong>?</v-card-text>
				<v-card-actions>
				<v-spacer/>
		...
		>
			<v-card>
				...
				<v-card-text>
					<v-textarea
							v-model.trim="editingPerson.comment"
							label="Комментарий"
							no-resize
							solo
					/>
		...
		>
			...
			<template #item.acceptanceDate="{item}">
				{{m(item.acceptanceDate).locale('ru').format('LLLL')}}
			</template>
...
export default class BooksDataTable extends Vue {
	...
	get callbackSpeciality() {
		return this.editingPerson.speciality ? this.editingPerson.speciality.title : '';
	}
	...
	openEditDialog(person: any) {
		this.editingPerson = { ...this.editingPerson, ...person, ...this.getDate(new Date(person.acceptanceDate)) };
	...
	async editBook() {
		const acceptanceDate = new Date(`${this.editingPerson.date} ${this.editingPerson.time}`);
		const body = {
	...
	async deleteBook() {
		...
		await this.$api.book.delete(`/${this.editingPerson._id}`)
							.then(({ data }) => eventBus.$emit(SHOW_ALERT, data.message, { type: 'success' }))
							.catch(({ response }) => eventBus.$emit(SHOW_ALERT, response?.data.message, { type: 'error' }));
